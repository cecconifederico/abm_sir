...
to update_infected [status_to_test probs]
        if modality = "mean_field"[
          let x one-of other turtles
          if (status_to_test = "I") and ([status] of x) = status_to_test and (random-float 1) < probs[
            set status "I"
          ]
          if (status_to_test = "S") and ([status] of x) = status_to_test and (random-float 1) < probs[
            ask x [set status "I"]
          ]
        ]
        if modality = "4neigh"[
...
"" ""
PENS
"default" 1.0 0 -5298144 true "" "plot (count turtles with [status = \"I\"]) / N"
"pen-1" 1.0 0 -14070903 true "" "plot (count turtles with [status = \"S\"]) / N"
"pen-2" 1.0 0 -14439633 true "" "plot (count turtles with [status = \"R\"]) / N"
