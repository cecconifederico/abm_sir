...
#define BETA 0.05                  //infection probability per meeting
...
#define S 2000                     //number of independent realizations
...
for (i=0;i<L;i++){
  ...
  for (s=0;s<S;s++){
    ...
    if ((fa=fopen("susceptibles.dat","w"))==NULL){
      printf ("Impossibile aprire il file-a \n");
      exit (1);
    }
    if ((fb=fopen("infected.dat","w"))==NULL){
      printf ("Impossibile aprire il file-b \n");
      exit (1);
    }
    if ((fc=fopen("recovered.dat","w"))==NULL){
      printf ("Impossibile aprire il file-c \n");
      exit (1);
    }
    if ((fd=fopen("susc_vs_inf.dat","w"))==NULL){
      printf ("Impossibile aprire il file-d \n");
      exit (1);
    }
    if ((fe=fopen("susc_vs_rec.dat","w"))==NULL){
